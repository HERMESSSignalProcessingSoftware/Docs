...
\pgfsetlayers{main,background,foreground}
...
%STAMP rechts
\node[black, right of = F1S1name, node distance = 12cm] (F1S4name) {\small{c STAMP \#4}};
...
\node[rectangle, draw, black, minimum height = 1cm, minimum width = 4.6 cm] at ($(F1S1cu)!.5!(F1S4cu) $) (F1MemCu){Controll Unit};
\node[above of = F1MemCu, node distance = 1cm] (F1MemName) {c Memory};
\node[rectangle, draw, black, minimum height = 1cm, minimum width = 4.6 cm, below of = F1MemCu, node distance = 1.5cm] (F1MemTG)  {Timestamp Generator};
\node[rectangle, draw, black, very thick, minimum height = 1cm, minimum width = 4.6cm, below of = F1MemTG, node distance = 1.5cm] (F1MemSynch) {Synch. Statemachine };
\node[rectangle, draw, black, minimum height = 1cm, minimum width = 4.6cm, below of = F1MemSynch, node distance = 1.5cm] (F1MemAmber) {AMBA Connector};
...
\path[red!50, draw, line width = 2pt] (ABS1) -- +(0cm, -0.3cm) -- +(2.3cm, -0.3cm) -- + (2.3cm, -1.7cm); 
\path[red!50, draw, line width = 2pt] ($(ABS1) + (2.3cm, -1.7cm)$) -- +(0cm, -7.7cm); 
\path[red!50, draw, line width = 2pt] (ABS2) -- +(0cm, -0.3cm) -- +(2.3cm, -0.3cm); 
\path[red!50, draw, line width= 2pt] ($(ABS2) + (2.3cm, -1cm) $) -- ($(ABS5) + (-2.3cm, -1cm)$);
\path[red!50, draw, line width = 2pt] (ABS3) -- +(0cm, -0.3cm) -- +(2.3cm, -0.3cm); 
...
\coordinate (AMBAT1) at ($(ABS2) + (2.3cm, -1cm) $);
\coordinate (AMBAT2) at ($(ABS5) + (-2.3cm, -1cm)$);
\coordinate (AMBAT3) at ($(AMBAT1) !.5! (AMBAT2)$);
\path[red!50, draw, line width = 2pt] ($(AMBAT1) !.5! (AMBAT2)$) -- ($(F1MemAmber) + (0cm, -.5cm) $);
\path[red!50, draw, line width=2pt] (AMBAT3) -- +(0cm,- 5cm);
%Interrupts
\coordinate (IntE) at ($(AMBAT3) - (.5cm, 5cm)$);
\coordinate (Int) at ($(F1MemAmber) - (0.5cm, 0.7cm)$);
\coordinate (IntE1) at ($(AMBAT3) - (1.5cm, 5cm)$);
\coordinate (Int1) at ($(F1MemAmber) - (1.5cm, 0.7cm)$);
\path[blue!70, draw, line width = 2pt] (Int1) -- (IntE1);
\path[blue!70, draw, line width = 2pt] (Int) -- (IntE);
% Reset Logic
...
\coordinate (rlC1) at ($(rl3) + (0cm, 3cm)$);
\coordinate (rlC2) at ($(rl6) + (0cm, 3cm)$);
\coordinate (rlC3) at ($(F1MemAmber) + (0.5cm, -.7cm)$);
%\coordinate (rlC3) at ($(compWatchdog) + (0cm, 2.2cm)$);
\path[gray!70, draw, line width = 1.5pt] (rlC1) -- (rlC2);
\path[gray!70, draw, line width = 1.5pt] (rlC3) -- + (0cm, -2.3cm);
...
\path[black, draw, line width = 3pt] (intCS6) -- +(-.55cm, 0cm);
...
\coordinate (memRight) at ($(compMemory) + (2.5cm, 0cm)$);
\path[black, draw, line width = 9pt] (memLeft) --+(-1.55cm,0cm);
\path[black, draw, line width = 9pt] (memRight) --+(1.4cm,0cm);
%\coordinate (compWtd1) at ($(compWatchdog) -(2.55cm, 0cm)$);
%\coordinate (compWtd2) at ($(compWatchdog) +(2.55cm, 0cm)$);
%\path[black, draw, line width = 3pt] (compWtd1) -- +(-0.5cm,0cm);
%\path[black, draw, line width = 3pt] (compWtd2) -- +(0.45cm,0cm);
%Legende
\begin{pgfonlayer}{foreground}
	\coordinate (leg) at ($(F1MemName) !.5! (F1S4name)$);
	\node[node distance = 3cm] at ($(leg) + (0cm, 2.5cm)$) (legende) {Legende}; 
	\coordinate (l1) at ($(legende) -  (0cm, .5cm)$);
	\coordinate (l2) at ($(legende) -  (0cm, 1cm)$);
	\coordinate (l3) at ($(legende) -  (-3cm, .5cm)$);
	\coordinate (l4) at ($(legende) -  (-3cm, 1cm)$);
	...
	\path[blue, draw, line width=2pt] (l4) -- +(1cm,0cm) node[right](intInt){Interrupts};
\end{pgfonlayer}
\begin{pgfonlayer}{background}
	\node[rectangle, draw, black, fill=white, fit=(legende) (intDB)(intInt), inner sep = 3mm, outer sep = 1mm, rounded corners = .5cm] (legendeall){};
\end{pgfonlayer}
\end{tikzpicture}
